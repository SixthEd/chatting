...
const db = {
    query: (...args) => {
        console.log(args);
        return new Promise((res) => res({ rows: [{ id: 1 }] }));
    },
};
...
router.get("/ws", (req, res) => {
    ...
    web.handleUpgrade(req, req.socket, new Buffer(""), (socket, incoming) => {
        ...
        socket.on("message", (message) => {
            ...
            const websocketConn = userToWsConn[parsedMessage.username];

            console.log("userToWsConn", userToWsConn);

            // if this exists
            if (websocketConn) {
                websocketConn.send(message.toString());
            }
